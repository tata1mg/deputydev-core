...
class UnifiedDiffApplicator:
    ...
    def do_replace(self, fname, content, hunk):
        fname = Path(fname)
    ...
    def process_fenced_block(self, block: List[str]):
        block.append("@@ @@")
    ...
    def hunk_to_before_after(self, hunk, lines=False):
        before = []
        after = []
        op = " "
        for line in hunk:
    ...
    def _apply_diff_to_file(self, filepath: str, diff: str) -> None:

    def apply_diff(self, filepath_to_diff_map: Dict[str, str]):
        for filepath, diff in filepath_to_diff_map.items():
            self._apply_diff_to_file(filepath, diff)
